...
//수정중.. 이미지 및 음악 파일 매칭, 로그인 관련부분 추가된거, 점수 관련 부분 추가해야함
...
//    private Image introBackground = new ImageIcon(getClass().getResource("/menu_images/Intro_background.jpg")).getImage();
    ...
    URL bakgroundGIF = getClass().getResource("../images/intro_background.gif");
    ...
    // memuBar 객체 안에 memuBar 이미지가 들어가게 됨 (기본적으로 있지만 멋을위한것인듯)  //넣을거면 넣고 아니면 빼도 될듯.
   private JLabel menuBar = new JLabel(new ImageIcon(getClass().getResource("/menu_images/menuBar.png")));
    ...
    private ImageIcon exitButtonImage = new ImageIcon(getClass().getResource("../images/exiButtonBasic.png"));
    private ImageIcon exitButtonEnteredImage = new ImageIcon(getClass().getResource("../images/exiButtonEntered.png"));


    // 시작버튼과 종료버튼 생성 startButtonEntered  //
    private ImageIcon startButtonBasic = new ImageIcon(getClass().getResource("/images/startButtonBasic.png"));
    private ImageIcon startButtonEntered = new ImageIcon(getClass().getResource("/images/startButtonEntered.png"));

    private ImageIcon quitButtonBasic = new ImageIcon(getClass().getResource("/images/quitButtonBasic.png"));
    private ImageIcon quitButtonEntered = new ImageIcon(getClass().getResource("/images/quitButtonEntered.png"));

    // 게임 화면에서 곡을 선택을 위한 화살표 버튼 이미지 / 이미지 만들어서 넣어줘야함.
    ...
    /* 게임 난이도 설정 버튼 우리는 난이도 하나로 할 거라 이거 없애고 시작버튼 하나 만들면 될 듯.
    ...
    */
    ...
    // backButton : 메인 화면으로 돌아가는 버튼  // 새로만들건 있던 거 다시쓰건 암튼 넣어줘야함.
   ...
   //음악. 참고로 우리가 넣은 음악을 프로젝트에서 실행시키려면 추가적인 jar 필요함.(build path)
   
   
   
    // track 클래스에 따른 리스트와 그 원소들. 얘를들어 0번 인덱스가 santa tell me 면, 그 안의 변수들은 그 노래의 타이틀 이미지. 게임이미지 등등
    ArrayList<Track> trackList = new ArrayList<Track>();

    // trackList 안에 있는 값에 따라서 아래 변수들의 값이 달라짐
    // nowSelected 값은 현재 선택된 트랙의 번호이자 arraylist 의 index 를 의미한다.
    private int nowSelected = 0;
    private Image selectedImage;//선택화면에서의 이미지(앨범커버 등)
    private Image titleImage; // 제목을 써준 이미지
    private Music selectedMusic; 

    // 인트로 음악 정의
    //private Music Intromusic = new Music("Intro_Elektronomia_Energy.mp3", true(무한반복.isLoop), "menu");
...
//    private Image scoreResult = new ImageIcon(getClass().getResource("/menu_images/scoreResult.png")).getImage();
    ...
    public void musicStart() {
        // trackList 에 track 정보 넣기, 트랙 클래스에서 정의한 대로 순서대로 해당변수의 파일이름 넣어줌. 뭐뭐넣을건지 상의필요. 
        ...
        //setUndecorated(true); // 기본 메뉴바 삭제 (메뉴바 커스텀하고 싶으면 이 코드 쓰고 paintcomponent, 메뉴바 이미지 넣으면됨 +setBounds)
        setTitle("Dynamic Music");
        setSize(Main.SCREEN_WIDTH, Main.SCREEN_HEIGHT);
        setResizable(false); // 한번 창이 생성되면 임의적으로 창 크기 변경 f불가
        ...
        // 게임 시작시 인트로 음악 재생 (없어도 될듯)
       //Intromusic.start();

        // exitButton   원래 있는 템플릿 안보이고 내가 커스텀한거 나오도록 하기 위한것
        ...
        exitButton.addMouseListener(new MouseAdapter() {
            ...
            public void mouseEntered(MouseEvent e) {
                ...
                exitButton.setCursor(new Cursor(Cursor.HAND_CURSOR)); //커서 이미지를 손가락 모양으로 변경
            }

            @Override // 버튼에 마우스 뗐을 때 이벤트
            public void mouseExited(MouseEvent e) {
                exitButton.setIcon(exitButtonImage);
                exitButton.setCursor(new Cursor(Cursor.DEFAULT_CURSOR)); //커서 이미지를 손가락에서 디폴트로 변경
            }
...
//        hardButton.setBounds(900, 350, 250, 67);
    ...
    public void paint(Graphics g) {
        screenImage = createImage(Main.SCREEN_WIDTH, Main.SCREEN_HEIGHT); // 메인클래스에서 설정한 화면크기만큼
        screenGraphic = screenImage.getGraphics(); // screenImage 를 이용해서 그래픽 객체를 얻어옴
//        
        screenDraw((Graphics2D) screenGraphic); // (그래픽스 2D 로 형변환)
//   
        g.drawImage(screenImage, 0, 0, null); // 윈도우 창에 screenImage 를 0,0 위치에 그려준다.
    }

    public void screenDraw(Graphics2D g) { // 매개변수를 Graphics 에서 Graphics2D 로 변경
//      
        // drawImage로 Introbackground 를  x, y 좌표에 그려줌
        g.drawImage(introBackground, 0, 0, null); //단순하게 이미지를 그려줌.
        //paintComponents(g); //메뉴바 등 추가해준 것까지 같이 그려줌 (어차피 위치 고정인 버튼이나 메뉴바 등은 이걸로 표현)
        this.repaint(); //?? 다시 panit 함수를 불러옴. 이렇게 매 순간마다 이미지를 계속 다시 그려줌.
...
//        System.out.println("d");
    ...
    public void selectTrack(int nowSelected) {
        ...
        titleImage = new ImageIcon(getClass().getResource("/images/" +
                trackList.get(nowSelected).getTitleImage())).getImage();
        selectedImage = new ImageIcon(getClass().getResource("/images/" +
                trackList.get(nowSelected).getStartImage())).getImage();
    ...
    //게임 시작하기 버튼을 눌렀을때, 인트로 화면이 사라지고 메인화면이 나타남. 시작하기 버튼 없어짐
...
//        introBackground = new ImageIcon(getClass().getResource("/menu_images/main_Bakground.jpg")).getImage();
        bakgroundGIF = getClass().getResource("/images/게임시작화면이미지이름.gif");
        introBackground = new ImageIcon(bakgroundGIF).getImage();
